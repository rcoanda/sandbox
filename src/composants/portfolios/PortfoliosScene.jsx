import { useDico } from '../../composants/globals/Dico'
import { portfolios, reelCode } from '../../config/PortfoliosConfig'

const LANG = {
  fr: {
    badge: '35MM',
    play: 'PROJETER CE SITE',
    frame: 'VUE',
    film: 'KODAK',
  },
  en: {
    badge: '35MM',
    play: 'PROJECT THIS SITE',
    frame: 'FRAME',
    film: 'KODAK',
  },
}

function PortfoliosScene() {
  const { lang } = useDico()
  const t = LANG[lang] || LANG.fr
  const total = portfolios.length

  return (
    <div className="p-stage">
      <section className="p-reel">
        {portfolios.map((site, i) => (
          <a
            key={site.id}
            className="p-frame"
            href={site.url}
            target="_blank"
            rel="noopener noreferrer"
            style={{ '--accent': site.accent }}
          >
            <div className="p-perf p-perf--left" aria-hidden="true" />
            <div className="p-frame__body">
              <div className="p-frame__head">
                <span>{t.film} — {t.frame} {String(i + 1).padStart(4, '0')}</span>
                <span>{t.badge} {reelCode(site.name)}</span>
              </div>

              <div
                className="p-screen"
                style={{ backgroundImage: `linear-gradient(155deg, ${site.accent} 0%, #050507 150%)` }}
              >
                <span className="p-screen__arrow" aria-hidden="true">▶</span>
                <span className="p-screen__name" style={{ color: site.accent }}>{site.name}</span>
                <span className="p-screen__play">{t.play}</span>
                <span className="p-screen__url">{site.url.replace(/^https?:\/\//, '')}</span>
              </div>

              <div className="p-frame__caption">
                <span className="p-frame__name">{site.name}</span>
                <span className="p-frame__reel">#{reelCode(site.name)}-{String(i).padStart(4, '0')}/{total}</span>
              </div>

              <div className="p-frame__edge">
                <span className="p-smpte" aria-hidden="true" />
                <span>{site.name.toUpperCase()} — {t.badge} — {i + 1}/{total}</span>
              </div>
            </div>
            <div className="p-perf p-perf--right" aria-hidden="true" />
          </a>
        ))}
      </section>
    </div>
  )
}

export default PortfoliosScene